...
class PProductCategories
{
    ...
    public function getListJH()
    {

        //Load the categories: id, title, tally-of-products
        $db = JFactory::getDBO();
        $db->setQuery("
			SELECT category_id as id, category_name as title, 0 as tally
			FROM #__hikashop_category
			WHERE (category_type = 'product') AND (category_published = 1)
		");
        $this->categories = $db->loadObjectList();

        //Load the category-category-child links: parent_category, child_category
        $db->setQuery("
			SELECT category_parent_id as parent_category, category_id as child_category
			FROM #__hikashop_category
			WHERE (category_type = 'product') AND (category_published = 1)
		");
        $links = $db->loadObjectList();
